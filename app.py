...
def run_project1():
    ...
    st.markdown("""
    **[포트폴리오 - 프로젝트 1]**  
    사내 ERP(지출결의) 데이터와 카드사 실승인 데이터를 대사하여 **미증빙, 금액 불일치, 내부규정 위반 결제**를 자동으로 탐지합니다.
    """)
    ...
    with col1: st.metric("총 지출결의 검토 건수", f"{len(filtered_df):,}건")
    with col2: st.metric("🚨 이상치 적발 건수", f"{len(filtered_anomalies):,}건")
    with col3:
        risk_rate = (len(filtered_anomalies) / len(filtered_df)) * 100 if len(filtered_df) > 0 else 0
        st.metric("내부통제 위반율", f"{risk_rate:.1f}%")
    with col4: st.metric("위반 의심 총 금액", f"₩{filtered_anomalies['ERP청구액'].sum():,.0f}")
...
def run_project2():
    ...
    st.markdown("""
    **[포트폴리오 - 프로젝트 2]**  
    각 사업부의 월별 제조원가 명세서를 분석하고, **표준원가 대비 실제원가 차이(Variance)를 검증**하여 월 결산의 정확도와 속도를 높입니다.
    """)
            ...
            for bu in bu_list:
                for cost_type, details in cost_structure.items():
                    for detail in details:
                        base_cost = np.random.randint(50, 500)
                        if bu == '메탈(합금철) BU' and detail == '전력비': base_cost *= 5 
                        if bu == '프레스 BU' and detail in ['열연강판/후판', '외주가공비']: base_cost *= 3
                            
                        variance_rate = np.random.uniform(-0.15, 0.25)
                        actual_cost = base_cost * (1 + variance_rate)
                        
                        data.append({
                            '결산월': f'2025-0{month}', '사업부': bu, '원가요소': cost_type,
                            '세부계정': detail, '표준원가(백만원)': int(base_cost), '실제원가(백만원)': int(actual_cost)
                        })
    ...
    with col_f1: selected_month = st.selectbox("결산월 선택", options=sorted(df['결산월'].unique(), reverse=True))
    with col_f2: selected_bu = st.multiselect("[결산] 사업부 선택", options=df['사업부'].unique(), default=df['사업부'].unique())

    filtered_df = df[(df['결산월'] == selected_month) & (df['사업부'].isin(selected_bu))]

    total_std = filtered_df['표준원가(백만원)'].sum()
    total_act = filtered_df['실제원가(백만원)'].sum()
    total_var = filtered_df['차이금액'].sum()
    
    col1, col2, col3, col4 = st.columns(4)
    with col1: st.metric("총 당기제품제조원가(실제)", f"{total_act:,.0f} 백만원", delta=f"{(total_var/total_std)*100:.1f}%" if total_std>0 else "0%", delta_color="inverse")
    with col2: st.metric("총 표준원가", f"{total_std:,.0f} 백만원")
    with col3: st.metric("원가 차이(불리한 차이)", f"{total_var:,.0f} 백만원")
    with col4: st.metric("🚨 결산 검증 요망 건수", f"{len(filtered_df[filtered_df['차이율(%)'] >= 20])} 건")

    st.divider()
    tab1, tab2, tab3 = st.tabs(["📊 사업부별 원가 분석", "🔍 원가요소별 비중", "⚠️ 결산 이상치 검증(원가차이)"])
    ...
    with tab2:
        ...
        with col_t1:
            fig2 = px.pie(filtered_df.groupby('원가요소')['실제원가(백만원)'].sum().reset_index(), values='실제원가(백만원)', names='원가요소', hole=0.4, template='plotly_white')
            st.plotly_chart(fig2, use_container_width=True)
        with col_t2:
            fig3 = px.treemap(filtered_df[filtered_df['원가요소'] == '제조경비'], path=['사업부', '세부계정'], values='실제원가(백만원)', template='plotly_white')
            st.plotly_chart(fig3, use_container_width=True)

    with tab3:
        def highlight_var(val):
            return 'background-color: #ffcccc' if val >= 30 else ('background-color: #fff2cc' if val >= 20 else '')
        st.dataframe(filtered_df[filtered_df['차이율(%)'] >= 20].sort_values(by='차이율(%)', ascending=False).style.map(highlight_var, subset=['차이율(%)']), use_container_width=True)
...
# ==============================================================================
st.sidebar.title("구민준 재무회계 포트폴리오")
st.sidebar.divider()

selected_project = st.sidebar.radio("📊 분석 프로젝트 선택", ("1. 내부통제 및 이상치 탐지 (증빙 대사)", "2. 제조원가 검증 및 결산 보조"))
st.sidebar.divider()
st.sidebar.info("본 대시보드는 SIMPAC의 비즈니스 구조를 모티브로 제작된 가상 데이터 기반 포트폴리오입니다.")

if selected_project == "1. 내부통제 및 이상치 탐지 (증빙 대사)": run_project1()
elif selected_project == "2. 제조원가 검증 및 결산 보조": run_project2()
